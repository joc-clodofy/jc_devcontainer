...
# Reintentos para operaciones Mongo (timeout/conexión/túnel SSH)
MONGO_OP_RETRIES = int(os.environ.get("MONGO_OP_RETRIES", "4"))
MONGO_OP_RETRY_DELAY = float(os.environ.get("MONGO_OP_RETRY_DELAY", "5"))

# sshtunnel referencia paramiko.DSSKey, eliminado en paramiko 3.x/4.x; parche para auth por password
try:
    import paramiko
    if not hasattr(paramiko, "DSSKey"):
        ...
        class _DSSKeyPlaceholder:
            ...
            def from_private_key_file(*args, **kwargs):
                raise NotImplementedError("DSA no soportado en esta version de paramiko; use password SSH")
...
# La URI y credenciales de Mongo se leen de Odoo (modelo parametrizacion: mongo_db_url, mongo_db).
# El servidor Colfisio tiene seguridad SSH: para ejecutar desde fuera debes usar túnel SSH hasta
# el servidor; las credenciales correctas son las que están en parametrizacion en Odoo.
# Solo define MONGO_URI si necesitas sobreescribir (ej. host distinto por túnel).
MONGO_URI_FALLBACK = os.environ.get("MONGO_URI", "mongodb://anasinf_user:Y34CeEdJNjut3fQm3lOKaQwpbtnW3c7@localhost:27017/?authSource=admin&readPreference=primary&directConnection=true&ssl=false")
MONGO_DB_NAME_FALLBACK = os.environ.get("MONGO_DB_NAME", "anasinf")
COL_EFECTOS = os.environ.get("MONGO_COL_EFECTOS", "efectos")


DRY_RUN = False
# Por defecto False: borrar en Odoo de una al tener reemplazo (no cola al final)
DEFER_ODOO_DELETE = os.environ.get(
   "INCOHERENT_REFUND_DEFER_ODOO_DELETE", "0"
).lower() in ("1", "true", "yes")
# Procesar como maximo N rectificativas por ejecucion (0 = sin limite)
BATCH_SIZE = int(os.environ.get("INCOHERENT_REFUND_BATCH_SIZE", "2000"))
...
# Por defecto: todos los colegiados (sin filtro). Para filtrar: env INCOHERENT_REFUND_CODIGOS_COLEGIADO o --codigos.
_codigos_env = os.environ.get("INCOHERENT_REFUND_CODIGOS_COLEGIADO", "")
...
# JSON-RPC Odoo: timeout (segundos) y reintentos ante timeout/red
...
def _jsonrpc_call(service: str, method: str, args: List[Any]) -> Any:
   """Llama al endpoint JSON-RPC de Odoo; reintenta ante timeout o fallos de red."""
...
def get_mongo_config_from_parametrizacion(execute_kw: Callable) -> Dict[str, Any]:
   """Lee Parametrizacion en Odoo (Conexiones Mongo): URI, DB, y si hay SSH: host, user, pass, puertos."""
   try:
       res = execute_kw(
           "parametrizacion",
           "search_read",
   ...
   row = res[0]
   return {
       "mongo_db_url": row.get("mongo_db_url") or MONGO_URI_FALLBACK,
       "mongo_db": row.get("mongo_db") or MONGO_DB_NAME_FALLBACK,
       "ssh_host": (row.get("ssh_host") or "").strip() or None,
       "ssh_user": row.get("ssh_user") or None,
       "ssh_pass": row.get("ssh_pass") or None,
       "ssh_port": str(row.get("ssh_port") or "22").strip() or "22",
       "mongo_port": int(row.get("mongo_port") or 27017),
   }


def _start_ssh_tunnel_and_mongo_uri(config: Dict[str, Any]) -> Tuple[str, str, Optional[Any]]:
   """
   Conexion a Mongo igual que en traspasodedatosmongodb (conexion.py):
   - Si hay ssh_host: tunel SSH al servidor y Mongo por puerto local.
   - Si no hay ssh_host o falla SSH: conexion directa con mongo_db_url (como extend).
   ...
   if not SSHTunnelForwarder:
       logger.warning("Parametrizacion tiene Ssh Host pero 'sshtunnel' no esta instalado. Usando conexion directa.")
       return mongo_db_url, mongo_db_name, None

   ssh_user = config.get("ssh_user") or ""
   ssh_pass = config.get("ssh_pass") or ""
   # Mismo estilo que conexion.py base: primer arg = host (puerto SSH por defecto 22)
   ssh_port = int(config.get("ssh_port") or 22)

   try:
       logger.info("Abriendo tunel SSH a %s (como en Parametrizacion / Conexiones Mongo)...", ssh_host)
       ...
       server.start()
       # Igual que Odoo: reemplazar puerto Mongo en la URL por el puerto local del tunel
       ...
       logger.warning("Error al abrir tunel SSH (%s). Usando conexion directa a Mongo como fallback.", e)
       # Como traspasodedatosmongodb_extend: fallback a mongo_db_url directo
       return mongo_db_url, mongo_db_name, None
...
def _codigo_to_int(value: Any) -> Optional[int]:
   """Convierte codigo_colegiado (int o str desde Odoo/JSON) a int para comparar con FILTER_CODIGOS_COLEGIADO."""
   if value is None:
       return None
   try:
       return int(value)
...
def _mongo_op_retry(func, *args, **kwargs):
   """Ejecuta func(*args, **kwargs) con reintentos ante fallos de red/timeout/cancelacion."""
   last_err = None
   for attempt in range(MONGO_OP_RETRIES):
       try:
           return func(*args, **kwargs)
       except (ConnectionFailure, AutoReconnect, OSError, TimeoutError) as e:
           last_err = e
           if attempt < MONGO_OP_RETRIES - 1:
               logger.warning("Mongo op fallo (%s), reintento %s/%s en %ss...", e, attempt + 1, MONGO_OP_RETRIES, MONGO_OP_RETRY_DELAY)
               time.sleep(MONGO_OP_RETRY_DELAY)
           else:
               raise
       except Exception as e:
           # OperationCancelled / _OperationCancelled (timeout o conexion cerrada)
           if "cancelled" in str(type(e).__name__).lower() or "cancelled" in str(e).lower():
               last_err = e
               if attempt < MONGO_OP_RETRIES - 1:
                   logger.warning("Mongo op cancelado/timeout (%s), reintento %s/%s en %ss...", e, attempt + 1, MONGO_OP_RETRIES, MONGO_OP_RETRY_DELAY)
                   time.sleep(MONGO_OP_RETRY_DELAY)
               else:
                   raise
           else:
   ...
   if last_err:
       raise last_err
   raise RuntimeError("mongo op retry unexpected")
...
def delete_refund_effects_in_mongo(col_efectos: Collection, refund_name: str) -> Tuple[int, str]:
   ...
   if DRY_RUN:
       return _mongo_op_retry(col_efectos.count_documents, query), "borrado_dry_run"
   result = _mongo_op_retry(col_efectos.delete_many, query)
...
def process(tunnel_holder: Optional[List] = None) -> Dict[str, Any]:
   """tunnel_holder: lista opcional donde anotar el tunel SSH para que el llamador lo cierre al salir."""
   print("[PROCESO] Conectando a Odoo (JSON-RPC)...")
   uid, execute_kw = get_odoo_jsonrpc()
   print("[PROCESO] Conectando a Mongo (URI y credenciales desde Parametrizacion / Conexiones Mongo en Odoo)...")
   config = get_mongo_config_from_parametrizacion(execute_kw)
   mongo_uri, mongo_db_name, tunnel = _start_ssh_tunnel_and_mongo_uri(config)
   if tunnel is not None and tunnel_holder is not None:
       tunnel_holder.append(tunnel)
   try:
       # Misma URI y opciones que en traspasodedatosmongodb (pymongo sync; Odoo usa motor async)
       client = MongoClient(
           mongo_uri,
       ...
       logger.error(
           "No se pudo conectar a MongoDB (URI/credenciales desde Parametrizacion).\n"
           "Si usas Ssh Host: el script abre tunel SSH como en Conexiones Mongo; si falla, hace fallback a conexion directa.\n"
           "Comprueba que la URL en Parametrizacion sea accesible desde esta maquina (tunel SSH o red)."
       )
   ...
   if FILTER_CODIGOS_COLEGIADO:
       ...
       to_fix = [it for it in to_fix if _codigo_to_int(it["refund"].get("partner_codigo_colegiado")) in codigos_set]
       # Prioridad: orden de FILTER_CODIGOS_COLEGIADO (ej. un solo colegiado primero)
       def _priority_key(item):
           cc = _codigo_to_int(item["refund"].get("partner_codigo_colegiado"))
           if cc is not None:
               try:
   ...
   if MAX_TO_PROCESS > 0:
       ...
       for item in to_fix:
           cc = _codigo_to_int(item["refund"].get("partner_codigo_colegiado"))
           if cc is not None and len(allowed_cc) < MAX_TO_PROCESS:
               allowed_cc.add(cc)
           if cc is not None and cc in allowed_cc:
               limited.append(item)
       to_fix = limited
       print(f"[PROCESO] Limite por colegiados: {MAX_TO_PROCESS} -> {len(allowed_cc)} colegiados, {len(to_fix)} rectificativas a procesar")


   total_incoherentes = len(to_fix)
   if not to_fix:
       print("[PROCESO] No hay rectificativas incoherentes que procesar.")
       return summary

   # Procesar de 2k en 2k (o BATCH_SIZE) por ejecucion
   if BATCH_SIZE > 0 and total_incoherentes > BATCH_SIZE:
       restantes = total_incoherentes - BATCH_SIZE
       to_fix = to_fix[:BATCH_SIZE]
       print(f"[PROCESO] Limite por ejecucion: {BATCH_SIZE} rectificativas (restantes en proxima ejecucion: {restantes})")
   ...
   for group_idx, group in enumerate(groups, 1):
       ...
       elapsed_minutes = int((time.time() - SCRIPT_START_TIME) // 60)
       print(f"procesando grupo {group_idx}/{len(groups)}")
       print(f"tiempo: {elapsed_minutes}min")
...
def main() -> None:
   ...
   tunnel_holder = []
   try:
       summary = process(tunnel_holder=tunnel_holder)
       ...
       for t in tunnel_holder:
           try:
               t.stop()
               logger.info("Tunel SSH cerrado.")
           except Exception as e:
               logger.warning("Al cerrar tunel SSH: %s", e)
...
if __name__ == "__main__":
   ...
   parser.add_argument("--codigos", type=str, default=None,
                       help="Solo procesar estos codigos colegiado (ej: 9769,9339). Sin esto ni env: todos.")
   ...
   if args.all_colegiados:
       ...
       # Sin --codigos ni --all-colegiados: procesar todos los colegiados (ignorar env)
       FILTER_CODIGOS_COLEGIADO.clear()
   try:
       main()
   except Exception:
